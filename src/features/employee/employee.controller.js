...
class EmployeeController {
  ...
  static async exportToPdf(req, res, next) {
    try {
      ...
      doc.fontSize(16).font('Helvetica-Bold').text('Relatório de Funcionários', { align: 'center' });
      doc.moveDown(0.5);
      const generationDate = new Date().toLocaleString('pt-BR');
      doc.fontSize(8).font('Helvetica').text(`Gerado em: ${generationDate}`, { align: 'center' });
      ...
      const columnRatios = {
        ...
        'Data de Admissão': 1.2, 'Nível de Formação': 1.5, 'Área de Formação': 1.5,
        'Data de Nascimento': 1.2, 'Gênero': 0.8, 'Estado Civil': 1.2, 'Possui Filhos': 0.8,
        'Número de Filhos': 0.8, 'CPF': 1.2, 'RG': 1, 'Órgão Emissor (RG)': 1,
        'Logradouro': 2.5, 'Número (Endereço)': 0.8, 'Complemento': 1.2, 'Bairro': 1.5,
        ...
        'Situação Funcional': 1.2, 'Observações Gerais': 3, 'Comorbidade': 1.5, 'Deficiência': 1.5,
        'Tipo Sanguíneo': 1
      };
